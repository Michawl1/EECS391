...
public class PlannerAgent extends Agent {
    ...
    private Stack<StripsAction> AstarSearch(GameState startState) {
    	...
    	Stack<GameState> closedList = new Stack<GameState>();
    	...
    	while(!openList.isEmpty())
    	{
    		...
    		for(GameState i : m.generateChildren())
    		{
    			if(!closedList.contains(i))
    			{
    				i.setCost(i.heuristic() + x);
    				openList.add(i);
    			}
    		}
    		x++;
    	...
    	Stack<StripsAction> finalList = new Stack<StripsAction>();
    	
    	GameState g = closedList.pop();
    	while(g.getParent() != null)
    	{
    		g = g.getParent();
    		finalList.push((StripsAction) g);
    	}
    	finalList.pop();
