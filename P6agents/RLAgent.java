...
import java.io.*;
		...
		public class RLAgent extends Agent {
		    ...
		    public double[] updateWeights(double[] oldWeights, double[] oldFeatures, double totalReward, State.StateView stateView, History.HistoryView historyView, int footmanId) 
		    {
		        ...
		        {
		        	newWeights[i] = oldWeights[i] + (gamma * calculateReward(stateView, historyView, footmanId) - oldFeatures[i]);
		        }
		    ...
		    public double calculateReward(State.StateView stateView, History.HistoryView historyView, int footmanId) {
		    	...
		    	for(DamageLog damageLogs : historyView.getDamageLogs(stateView.getTurnNumber()-1)) {
		    	     if(damageLogs.getAttackerController() == 1)
		    	    	 reward = reward + damageLogs.getDamage();
		    	     else
		    	    	 reward = reward - damageLogs.getDamage();
		    	     }
		    	
		    	for(DeathLog deathLog : historyView.getDeathLogs(stateView.getTurnNumber()-1)) {
		    		if(deathLog.getController() == 1)
		    	    	 reward = reward + 100;
		    	     else
		    	    	 reward = reward - 100;
		    	     }
		                             ...
		                             int defenderId) 
		    {
		        ...
		        qValue = learningRate * (calculateReward(stateView, historyView, attackerId)) + (gamma * features[maxIndex]);
		    ...
		    public double[] calculateFeatureVector(State.StateView stateView,
		                                           ...
		                                           int defenderId) {
		        /*
		         * Feature Vector will have HP, attack, and range of the attacker and defender
		         * Super basic implementation but these are the most important numbers for 
		         * both footmen 
		         */
		    	UnitView attacker = stateView.getUnit(attackerId); //attacker
		    	double attackerHP = attacker.getHP();
		    	double attackerBasicAttack = attacker.getTemplateView().getBasicAttack();
		    	double attackerRange = attacker.getTemplateView().getRange();
		    	
		    	UnitView defender = stateView.getUnit(defenderId); //defender
		    	double defenderHP = defender.getHP();
		    	double defenderBasicAttack = defender.getTemplateView().getBasicAttack();
		    	double defenderRange = defender.getTemplateView().getRange();
		    	
		    	double[] featureVector = {attackerHP, attackerBasicAttack, attackerRange,
		    							  defenderHP, defenderBasicAttack, defenderRange};
